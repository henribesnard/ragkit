...
@pytest.mark.asyncio
async def test_query_analyzer_unknown_intent_falls_back_to_question():
    llm = DummyLLM(
        json_response={
    ...
    config = QueryAnalyzerConfig(
        ...
        behavior=QueryAnalyzerBehaviorConfig(
            always_retrieve=False,
            detect_intents=["question", "greeting"],
            query_rewriting=QueryRewritingConfig(enabled=True, num_rewrites=1),
        ),
        system_prompt="prompt",
        output_schema={},
    ...
    agent = QueryAnalyzerAgent(config, llm)
    result = await agent.analyze("Capital?")
...
@pytest.mark.asyncio
async def test_query_analyzer_unknown_greet_intent_falls_back_to_greeting():
    llm = DummyLLM(
        json_response={
            "intent": "greet_user",
    ...
    agent = QueryAnalyzerAgent(config, llm)
    result = await agent.analyze("Salut")
