    ...
    <div className="space-y-6">
      <Card>
        ...
        <div className="mt-6 space-y-3">
          {isLoading && <p className="text-sm text-muted">{t('logs.loading')}</p>}
          ...
          {data.map((item: QueryLogEntry, index: number) => (
            <div key={`${item.timestamp}-${index}`} className="rounded-2xl bg-canvas p-4 text-sm">
              <div className="flex items-start justify-between gap-4">
                ...
                >
                  {item.success ? t('logs.status.ok') : t('logs.status.error')}
                </span>
              </div>
              <div className="mt-2 flex items-center justify-between text-xs text-muted">
                <span>
                  {t('logs.intent')}: {item.intent || t('common.values.unknown')}
                </span>
        ...
        {!isLoading && (
          <div className="mt-4 flex items-center justify-between">
            ...
            <span className="text-xs text-muted">{t('logs.page', { page: page + 1 })}</span>
            <Button
              variant="outline"
              size="sm"
